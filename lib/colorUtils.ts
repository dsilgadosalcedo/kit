import type DdssColorWithOnToken from "@/lib/types/ddss-color";
import type DdssColor from "@/lib/types/ddss-color";
import { DdssColors } from "@/lib/utils/debugUtils";
...
export function getColorsWithoutOnTokens(): DdssColor[] {
  return DdssColors.filter((color) => !colorsWithOnTokens.includes(color as DdssColorWithOnToken));
}

export function getOnToken(colorToken: DdssColor) {
  //check if the token has an on-token in the first place

  // if (!colorsWithOnTokens.includes(colorToken as DdssColorWithOnToken)) {
  ...
  function getFixedColorComplement(color: DdssColor) {
