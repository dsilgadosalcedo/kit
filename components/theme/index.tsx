...
const [colorMode, setColorMode] = useState<"light" | "dark">("light");
  ...
  useEffect(() => {
    ...
    Object.keys(theme.light).forEach((key) => {
      root.style.setProperty(`--light__${key.toLowerCase()}_ddssv`, theme.light[key]);
    });

    Object.keys(theme.dark).forEach((key) => {
      root.style.setProperty(`--dark__${key.toLowerCase()}_ddssv`, theme.dark[key]);
    });

    if (colorMode === "dark") {
      Object.keys(theme.dark).forEach((key) => {
        root.style.setProperty(`--light__${key.toLowerCase()}_ddssv`, theme.dark[key]);
      });
    }
  }, [theme, colorMode]);
