...
import { DdssTextProps } from "@/components/text";
import { DdssButtonProps } from "@/components/button";
import { DdssIconButtonProps } from "@/components/icon-button";
import { getOnToken } from "@/lib/colorUtils";
import Card from "@/components/card";
import Row from "@/components/row";
import { DdssCardProps } from "@/components/card";
import Icon, { DdssIconProps } from "@/components/icon";

interface DdssSnackbarProps extends React.HTMLAttributes<HTMLDivElement> {
  globalColor?: DdssColorWithOnToken;
  message?: string;
  children?: React.ReactNode;
  cardProps?: DdssCardProps;
}

/**
 * A snackbar component that displays temporary notifications with optional action buttons.
 */
export default function Snackbar(props: DdssSnackbarProps) {
  ...
  const getComponentName = (type: React.ComponentType<unknown> | string): string => {
    if (typeof type === "string") return type;
    return (type as React.ComponentType<unknown>)?.displayName || (type as React.ComponentType<unknown>)?.name || "Unknown";
  };
  ...
  return (
    <div data-ddss-component="snackbar" {...dataAttrs} {...restProps}>
      <Card
      ...
      >
        <Row alignItems="center">
          ...
          {icon && (
            <div data-ddss-slot="snackbar-icon">
              {globalColor
                ? React.cloneElement(icon, { color: getOnToken(globalColor), strokeWidth: 1.75 } as DdssIconProps)
                : icon}
            </div>
          )}
          ...
          {/* Message slot */}
          {text.length > 0 && (
            <div data-ddss-slot="snackbar-text">
              {text.map((text, index) =>
                globalColor
                  ? React.cloneElement(text, {
                      key: index,
                      color: getOnToken(globalColor as DdssColor) as DdssColor,
                      fontClass: "body",
                      className: getMargin(),
                    } as DdssTextProps)
          ...
          <Row>
            {buttons.length > 0 && (
              <div data-ddss-slot="snackbar-actions">
                {buttons.map((button, index) =>
                  globalColor
                    ? React.cloneElement(button, {
                        ...
                        style: {
                          backgroundColor: `rgb(from var(--ddss-${getOnToken(globalColor)}) r g b / 0.1)`,
                          border: `1px solid rgb(from var(--ddss-${getOnToken(globalColor)}) r g b / 0.2)`,
                          marginRight: !icon && "calc(-1em * pow(var(--ddss-wholestep-dec), 2))",
                        },
                        stateLayerOverride: { bgColor: `on${globalColor}` },
                      } as Partial<DdssButtonProps>)
                    : React.cloneElement(button, {
                        key: index,
                        size: "sm",
                        variant: "outline",
            ...
            {iconButtons.length > 0 && (
              <div data-ddss-slot="snackbar-icon-actions">
                {iconButtons.map((iconButton, index) =>
                  globalColor
                    ? React.cloneElement(iconButton, {
                        ...
                        color: globalColor,
                        fontClass: "heading",
                      } as Partial<DdssIconButtonProps>)
                    : React.cloneElement(iconButton, {
                        key: index,
                        fontClass: "heading",
                      } as Partial<DdssIconButtonProps>)
