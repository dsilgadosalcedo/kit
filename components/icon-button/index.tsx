...
declare global {
  type DdssIconButtonSize = "xs" | "sm" | "md" | "lg" | "xl";
}

interface DdssIconButtonBaseProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  icon: IconName;
  size?: DdssIconButtonSize;
  fontClass?: DdssFontClass; //optional, if present it will control the size directly via fontClass
  className?: string; //optional, making explicit here so we can control how it mixes in with locally-passed props
}

type IconButtonColor<T extends "fill" | "outline" | "text"> = T extends "fill" ? DdssColorWithOnToken : DdssColor;

export interface DdssIconButtonProps<T extends "fill" | "outline" | "text" = "fill"> extends DdssIconButtonBaseProps {
  icon: IconName;
  variant?: T;
...
export default function IconButton<T extends "fill" | "outline" | "text" = "fill">({
  ...
  return (
    <button
      data-ddss-component="icon-button"
      type="button"
      {...dataAttrs}
      {...restProps}
