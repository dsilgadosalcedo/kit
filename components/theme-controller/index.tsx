...
export default function ThemeController() {
  ...
  const brandPalette: DdssColorGroup[] = ["primary", "secondary", "tertiary"];

  const semanticPalette: DdssColorGroup[] = ["error", "warning", "success", "info"];

  const layoutPalette: DdssColorGroup[] = ["neutral", "neutralvariant"];

  useEffect(() => {
    updateTheme(palette);
  }, [palette, updateTheme]);

  const handleColorChange = (key: DdssColorGroup, newValue: string) => {
  ...
  return (
    <>
        ...
        <div className="position-fixed" style={{ zIndex: 1000 }}>
          <IconButton
            ...
            style={{
              position: "fixed",
              zIndex: 1000,
              top: "var(--ddss-size-xs)",
              left: "var(--ddss-size-xs)",
            }}
          />
        </div>
      )}
      {isOpen &&
        createPortal(
          <div className="ddss-theme-drawer">
            <Card
              scaleFactor="heading"
...
`}
      ...
      <style jsx>{`
        input[type="color"] {
          ...
          -moz-appearance: none;
          appearance: none;
          width: var(--ddss-size-lg);
          height: var(--ddss-size-lg);
          flex-basis: auto;
          flex: 0 0 auto;
          background-color: transparent;
          border: none;
          cursor: pointer;
          outline: 2px solid var(--ddss-onsurface);
          border-radius: 100em;
        ...
        .ddss-theme-drawer {
          ...
          width: calc(var(--ddss-size-4xl) * var(--ddss-wholestep));
          z-index: 1000;
          padding: var(--ddss-size-md);
        }
      `}</style>
    </>
  );
