...
import { DdssStateLayerProps } from "@/components/state-layer";
import Icon from "@/components/icon";

export interface DdssButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  label?: string;
  ...
  modifiers?: string;
  stateLayerOverride?: DdssStateLayerProps; // Optional override for state layer properties
}
...
export default function Button({
  ...
  const onColorToken = getOnToken(color) as DdssColor;
  ...
  const localStateLayerProps: DdssStateLayerProps = getLocalStateLayerProps();
  ...
  return (
    <button
      ...
      {...restProps}
      type="button"
      data-ddss-component="button"
      className={`${baseButtonClasses} ${modifiers || ""}`}
    >
      <div data-ddss-button-content-wrap="true">
        {startIcon && (
          <div data-ddss-icon-position="start">
            <Icon name={startIcon} color={variant === "fill" ? onColorToken : color} data-ddss-icon-position="start"></Icon>
          </div>
        )}
        <span data-ddss-button-child="button-text">{label ?? "Button"}</span>
        {endIcon && (
          <div data-ddss-icon-position="end">
            <Icon name={endIcon} color={variant === "fill" ? onColorToken : color} data-ddss-icon-position="end"></Icon>
          </div>
        )}
      </div>
